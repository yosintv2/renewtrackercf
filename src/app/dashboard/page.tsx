...
const CATEGORY_BAR: Record<string, string> = {
  ...
  living: "bg-green-400",
  tech: "bg-blue-400",
  lifestyle: "bg-orange-400",
  financial: "bg-red-400",
  other: "bg-gray-300",
};
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const [{ data: authData }, { data: profileData }, { data: subData }] = await Promise.all([
    ...
    <div className="space-y-5 max-w-2xl mx-auto lg:max-w-none">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">
            {subs.length === 0
              ...
              : overdue.length > 0
              ? `${overdue.length} overdue · ${upcoming.length} due this month`
              : upcoming.length > 0
              ? `${upcoming.length} payment${upcoming.length !== 1 ? "s" : ""} due this month`
              : "All payments are on track"}
          </p>
        </div>
        <Link
          href="/dashboard/subscriptions"
          className="flex items-center gap-1.5 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold px-4 h-10 rounded-xl shadow-md shadow-blue-200 transition-colors flex-shrink-0"
        >
      ...
      {subs.length === 0 && (
        <div className="bg-white rounded-2xl border-2 border-dashed border-gray-200 flex flex-col items-center justify-center py-20 text-center px-6">
          ...
          <h3 className="text-base font-bold text-gray-900 mb-2">Nothing tracked yet</h3>
      ...
      {subs.length > 0 && (
        <>
          ...
          <div className="bg-gray-900 rounded-2xl p-5">
            ...
            <div className="flex items-center gap-3 mt-2">
              <span className="text-sm text-gray-400">
                {fmt(Math.round(totalYearly))}/year
              </span>
              {dueSoon.length > 0 && (
                <span className="text-xs font-semibold bg-orange-500/20 text-orange-300 px-2 py-0.5 rounded-full">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              { label: "Total", value: subs.length, icon: Calendar, color: "text-blue-600", bg: "bg-blue-50" },
              { label: "Overdue", value: overdue.length, icon: AlertTriangle, color: "text-red-600", bg: "bg-red-50" },
              { label: "Due soon", value: dueSoon.length, icon: Clock, color: "text-orange-600", bg: "bg-orange-50" },
            ].map((stat) => (
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="flex items-center justify-between px-4 py-4 border-b border-gray-50">
              ...
              <Link href="/dashboard/reminders" className="text-xs text-blue-600 font-medium hover:underline">
                View all
              </Link>
            </div>
            ...
            <div className="divide-y divide-gray-50">
              {[...overdue, ...upcoming].length === 0 ? (
                ...
                [...overdue, ...upcoming].slice(0, 6).map((s) => {
                  ...
                  return (
                    <div key={s.id} className="flex items-center justify-between px-4 py-3.5 hover:bg-gray-50 transition-colors">
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-900 truncate">{s.name}</p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          {fmt(s.price)} · {formatDate(s.next_billing_date)}
                        </p>
                      </div>
                      <DaysTag days={days} />
                    </div>
          ...
          {byCategory.length > 0 && (
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-4 border-b border-gray-50">
                    ...
                    <div key={cat}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <span className="text-sm font-bold text-gray-900">
                          {fmt(Math.round(amount))}<span className="text-xs font-normal text-gray-400">/mo</span>
                        </span>
                      </div>
                      <div className="h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          className={cn("h-full rounded-full transition-all", CATEGORY_BAR[cat] ?? "bg-gray-400")}
                          style={{ width: `${pct}%` }}
                        />
                      </div>
                    </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-4 space-y-2">
            ...
            {[
              { href: "/dashboard/subscriptions", label: "Manage all subscriptions" },
              { href: "/dashboard/reminders", label: "View all reminders" },
