...
import { Menu, Shield, ChevronDown, Check, X, LogOut, ChevronRight, LayoutDashboard, Bell, Settings, Tv, Receipt } from "lucide-react";
  ...
  return (
    <aside className="fixed left-0 top-0 bottom-0 w-64 bg-white border-r border-gray-100 flex flex-col z-40">
      <div className="h-14 flex items-center justify-between px-5 border-b border-gray-100 flex-shrink-0">
        ...
        <button className="lg:hidden p-1.5 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100" onClick={onClose} aria-label="Close menu">
          <X className="w-4 h-4" />
        </button>
      </div>
          ...
          {navItems.map((item) => {
            ...
            return (
              <li key={item.href}>
                <a
                  href={item.href}
                  className={`group flex items-center gap-3 px-3 py-3 rounded-xl text-sm font-medium transition-all ${active ? "bg-blue-600 text-white shadow-md shadow-blue-200" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
                >
      ...
      <div className="p-3 border-t border-gray-100 flex-shrink-0">
        <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl hover:bg-gray-50 transition-colors">
          ...
          <button onClick={handleLogout} className="p-1.5 rounded-lg text-gray-300 hover:text-red-500 hover:bg-red-50 transition-colors" title="Sign out">
    ...
    <div className="min-h-screen bg-gray-50 flex w-full overflow-x-hidden">
      ...
      <div className="flex-1 min-w-0 flex flex-col min-h-screen lg:ml-64">
        <header className="h-14 bg-white border-b border-gray-100 flex items-center px-4 sm:px-6 sticky top-0 z-20 gap-3">
          <button className="lg:hidden p-2 rounded-lg text-gray-500 hover:text-gray-900 hover:bg-gray-100 transition-colors" onClick={() => setSidebarOpen(true)} aria-label="Open menu">
            <Menu className="w-5 h-5" />
          </button>

          <a href="/" className="lg:hidden flex items-center gap-1.5 font-bold text-base">
...
export default function DashboardShell({ children, currentPage }: { children: React.ReactNode; currentPage: string }) {
  return (
    <CurrencyProvider>
      <Shell currentPage={currentPage}>{children}</Shell>
    </CurrencyProvider>
  );
}
